...
Interpreter.intializers = [new ScriptFile('./runtime.js')];

var _emit = process.EventEmitter.prototype.emit;

inherit(Interpreter, process.EventEmitter, [
  function emit(event){
    if (this._events['*']) {
  ...
  function execute(subject, Ω){
    ...
    function control(context, next, result){
      ...
      if (result instanceof Thrown) {
        script.error = result.thrown;
        self.emit('exception', result.thrown);
      } else {
        script.result = result;
        self.emit('complete', result);
      }
...
inherit(FunctionExpressionThunk, FunctionDeclarationThunk, [
  ...
  function construct(context, args, Ω, ƒ){
    ...
    evaluate(this.body, context, function(result){
      Ω(isObject(result) ? result : context.receiver);
    }, constructReturnHandler(instance, Ω, ƒ));
  }
]);
...
var BuiltinArray, BuiltinBoolean, BuiltinDate, BuiltinError, BuiltinFunction, BuiltinObject,
...
var builtins = (function(builtins){
  ...
  Object.getOwnPropertyNames(Date.prototype).forEach(function(name){
    ...
    if (typeof builtin !== 'function' || builtin === Date) return;
  ...
  ['getOwnPropertyNames', 'getOwnPropertyDescriptor', 'defineProperty',
   ...
   'isExtensible', 'isFrozen', 'isSealed', 'preventExtensions', 'seal'].forEach(function(name) {
    ...
    BuiltinObject.functions.push(api(name, function(context, args, Ω, ƒ){
      Ω(builtin.apply(null, args));
    }));
  });
...
function isStopIteration(context, signal){
  if (signal instanceof Thrown) {
    if (context.StopIteration === signal.thrown)
      return true;
    if (isPrototypeOf.call(context.StopIterationPrototype, signal.thrown))
      return true;
  }
  return false;
}
...
var evaluaters = {
  ...
  FunctionDeclaration: function(node, context, Ω, ƒ){
    ...
    context.declare('function', node.id.name, node.thunk.instantiate(context));
    Ω();
  },
    ...
    evaluate(node.block, context, Ω, function(_, __, signal){
      if (signal instanceof Thrown) {
        iterate(node.handlers, function(node, next){
          var catchContext = new CatchScope(context, node.param.name, signal.thrown);
