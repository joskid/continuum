var continuum = (function(GLOBAL, exports, undefined){
  ...
  function ClassDefinitionEvaluation(name, superclass, constructor, methods){
    if (superclass === undefined) {
      ...
      if (superclass && superclass.IsCompletion) {
        if (superclass.IsAbruptCompletion) {
          return superclass;
        } else {
      ...
      if (superclass === null) {
        ...
        if (superproto && superproto.IsCompletion) {
          if (superproto.IsAbruptCompletion) {
            return superproto;
          } else {
    ...
    if (ctor && ctor.IsCompletion) {
      if (ctor.IsAbruptCompletion) {
        return ctor;
      } else {
        ctor = ctor.value;
      }
  ...
  void function(makeMultiplier){
    MUL = makeMultiplier(function(l, r){ return r * l });
    DIV = makeMultiplier(function(l, r){ return r / l });
    MOD = makeMultiplier(function(l, r){ return r % l });
    SUB = makeMultiplier(function(l, r){ return r - l });
  }(function(finalize){
  ...
  var ElementSuper, CallSuperSetup;

  void function(makeSuper){

    ElementSuper = makeSuper(function(env, prop){
    ...
    CallSuperSetup = makeSuper(function(env, prop){
      return env.GetMethodName();
    });

  }(function(getKey){
    return function(prop){
      var env = SuperGuard();
      if (env.IsAbruptCompletion) {
        return env;
      }
  ...
  inherit($Array, $Object, {
    ...
    DefineOwnProperty: function DefineOwnProperty(key, desc, strict){
      ...
      if (key === 'length') {
        ...
        if (newLen.IsCompletion) {
          if (newLen.IsAbruptCompletion) {
            return newLen;
          } else {
            newLen = newLen.Value;
          }
        ...
        if (success.IsCompletion) {
          if (success.IsAbruptCompletion) {
            return success;
          } else {
        ...
        while (newLen < oldLen) {
          ...
          if (deleted.IsCompletion) {
            if (deleted.IsAbruptCompletion) {
              return deleted;
            } else {
  ...
  function createThunk(code){
    ...
    function CLASS_DECL(){
      ...
      a = ClassDefinitionEvaluation(ops[ip][0], b, ops[ip][2], ops[ip][3]);
      if (a && a.IsCompletion) {
        if (a.IsAbruptCompletion) {
          error = a.value;
          return ƒ;
  ...
  function Continuum(listener){
    ...
    this.state = 'idle';
    this.eval(new NativeScript(__dirname+'/builtins.js'), true);
  }
