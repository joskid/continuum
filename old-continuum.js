var continuum = (function(GLOBAL, exports){
      ...
      SEQUENCE([
        ...
        function(local, Ω){
          ...
          FunctionDeclarationInstantiation(self, args, local, function(){
            interpreter.emit('enter-function', context);
            evaluate(self.Code, function(result){
              if (result && result.type === RETURN) {
                interpreter.emit('return', result);
                Ω(result.value);
              } else {
                Ω(result);
              }
  ...
  var evaluaters = {
    ...
    Identifier: function(node, Ω, ƒ){
      IdentifierResolution(node.name, Ω, ƒ);
    },
  ...
  function Realm(){
    ...
    for (var k in atoms)
      this.global.defineDirect(k, atoms[k], ___);
  }
  ...
  var realm = EC.realm = null,
      global = EC.global = null,
      context = EC.context = null,
      intrinsics = EC.intrinsics = null;
  ...
  define(EC, [
    function update(){
      if (!context) {
        ...
        realm = EC.realm = context.realm;
        if (realm) {
          global = EC.global = realm.global;
          intrinsics = EC.intrinsics = realm.intrinsics;
        }
      }
    },
    function push(newContext){
      context = EC.context = newContext;
      EC.update();
    },
    function pop(){
      if (context) {
  ...
  //var intializers = [new ScriptFile(__dirname + '/runtime.js')];
  var interpreter;
